...
/// variables in the `Assert` namespace. If `SF_SAFE_ASSERT_SAVES_FAIL_SITE` was
/// not defined at compile time, the macro has no effect.
///
...
/// halting the program.
///
/// This macro should only be used within functions that return `Result`. If
/// `SF_SAFE_ASSERT_SAVES_FAIL_SITE` was defined at compile time, the macro
/// saves the location of the failing assert in `Assert::failFile` and
...
///
/// @remark Safe asserts are most commonly used to assert invariants and safe
/// pointer dereferences in framework code. See
/// `docs/error-handling-methodology.md` for more info on the framework error
/// handling methodology.
///
/// @param[in] kExpr  Expression to assert. If untrue, returns `E_ASSERT` from
